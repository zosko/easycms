...
<div id="main">
	...
	<div id="middle">
		...
		<div id="center-column">
			<?php
				if($HTTP_GET_VARS['akcija'] == 'dodadi_anketa')
				{
				...
				if($HTTP_GET_VARS['akcija'] == 'anketi_lista')
				{
				...
				if($HTTP_GET_VARS['akcija'] == 'anketa_promeni')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("SELECT * FROM anketa WHERE id='".$HTTP_GET_VARS['id']."'");
					...
					echo '<tr><td ><b>Question:</b></td><td><input type="text" name="prasanje" id="prasanje" value="'.$rowz['prasanje'].'"/></td></tr>';
					$brojac = 1;
					while($row = $konektor->fetchArray($izlez))
					{
						echo '<tr><td><b>Answer '.$brojac.'</b></td><td class="first style2" width="100"><input  type="text" name="odgovor[]" id="odgovor" value="'.$row['odgovor'].'" /></td>
				...
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("DELETE FROM anketa WHERE id='".$HTTP_GET_VARS['id']."'");
				
					$konektor = new baza_konektor();
					$izlez = $konektor->query("INSERT INTO anketa(prasanje,objavi) VALUES ('".addslashes($_POST['prasanje'])."','1')"); 
					...
					{
						$izlez = $konektor->query("INSERT INTO anketa_prasanja(id_prasanje,odgovor) VALUES ('".$idto_od_anketata."','".addslashes($value)."')"); 
					}
				...
				if($HTTP_GET_VARS['akcija'] == 'anketa_neobjavi')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("UPDATE anketa SET objavi='0' WHERE id='".$HTTP_GET_VARS['id']."'"); 
					if($izlez)
					{
						echo "<script type=\"text/javascript\">jAlert('success', 'Successfully publish.', 'Success');</script>";
					}
				...
				if($HTTP_GET_VARS['akcija'] == 'anketa_objavi')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("UPDATE anketa SET objavi='1' WHERE id='".$HTTP_GET_VARS['id']."'"); 
					if($izlez)
					{
						echo "<script type=\"text/javascript\">jAlert('success', 'Successfully publish.', 'Success');</script>";
					}
				...
				if($HTTP_GET_VARS['akcija'] == 'anketa_izbrisi')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("DELETE FROM anketa WHERE id='".$HTTP_GET_VARS['id']."'");
					if($izlez)
				...
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("INSERT INTO anketa(prasanje,objavi) VALUES ('".addslashes($_POST['prasanje_anketa'])."','".$_POST['objavi_vednas_anketa']."')"); 
				...
				if($HTTP_GET_VARS['akcija'] == 'anketa_izbrisi_odgovor')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("DELETE FROM anketa_prasanja WHERE id='".$HTTP_GET_VARS['id']."'");
					if($izlez)
					{
						echo "<script type=\"text/javascript\">jAlert('success', 'Successfully publish.', 'Success');</script>";
					}
