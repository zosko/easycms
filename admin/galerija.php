...
<div id="main">
	...
	<div id="middle">
		...
		<div id="center-column">
			<?php
				if(isset($_GET['akcija']) && $_GET['akcija'] == 'dodadi_galerija')
				{
					echo '<form enctype="multipart/form-data" method="post" action="">
							<div class="table">
								<table class="listing" cellpadding="0" cellspacing="0">
									...
									<tr><td class="first style2">Publish now:</td><td class="first style2"><select name="objavi_vednas"><option value="0">Yes</option><option value="1">No</option></select></td></tr>
								</table>
								<input type="submit" name="dodadi_galerijata" id="dodadi_galerijata" value="Add gallery" />
							</div>
						</form>';
				}
				if(isset($_GET['akcija']) && $_GET['akcija'] == 'editiraj_galerija')
				{
				$konektor = new baza_konektor();
				$res = $konektor->query("SELECT COUNT(id) FROM galerija");	
				...
				if(isset($_GET['akcija']) && $_GET['akcija'] == 'edit_galerija')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("SELECT * FROM galerija_sliki WHERE id_galerija='".$_GET['id']."' ORDER BY ID DESC");
					$id_gallerija = $_GET['id'];
				...
				if(isset($_POST['dodadi_galerijata']))
				{
					...
					$konektor = new baza_konektor();
					$izlez = $konektor->query("INSERT INTO galerija(ime,objavi,naslovna) VALUES ('".$_POST['ime']."','".$_POST['objavi_vednas']."','')");
						...
						if(!empty($value))
						{
							...
							else
							{
								...
								if($bravo)
								{
									...
									echo "Успешно прикачена слика на сервер ".$ime."<br />";
								}
								else
								{
									echo "Грешка во качување слика на сервер<br />";
								}
							}
						}
						else
				...
				if(isset($_GET['akcija']) && $_GET['akcija'] == 'neobjavi_galerija')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("UPDATE galerija SET objavi='0' WHERE id='".$_GET['id']."'"); 
					if($izlez)
					{
				...
				if(isset($_GET['akcija']) && $_GET['akcija'] == 'objavi_galerija')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("UPDATE galerija SET objavi='1' WHERE id='".$_GET['id']."'"); 
					if($izlez)
					{
						echo "<script type=\"text/javascript\">jAlert('success', 'Successfully publish.', 'Success');</script>";
					}
				...
				if(isset($_GET['akcija']) && $_GET['akcija'] == 'naslovna_slika_galerija')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("SELECT * FROM galerija_sliki WHERE id='".$_GET['id']."'"); 
					$row = $konektor->fetchArray($izlez);
					$slikata_ime = $row['slika'];
					
					$konektor = new baza_konektor();
					$izlez = $konektor->query("UPDATE galerija SET naslovna='".$slikata_ime."' WHERE id='".$_GET['id_galerija']."'"); 
				...
				if(isset($_GET['akcija']) && $_GET['akcija'] == 'brisi_slika_galerija')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("DELETE FROM galerija_sliki WHERE id='".$_GET['id']."'");
					if($izlez)
					{
						echo "<script type=\"text/javascript\">jAlert('success', 'Successfully publish.', 'Success');</script>";
					}
				...
				if(isset($_GET['akcija']) && $_GET['akcija'] == 'brisi_galerija')
				{
					$konektor = new baza_konektor();
					$izlez = $konektor->query("DELETE FROM galerija WHERE id='".$_GET['id']."'");
					if($izlez)
					{
						echo "<script type=\"text/javascript\">jAlert('success', 'Successfully publish.', 'Success');</script>";
					}
